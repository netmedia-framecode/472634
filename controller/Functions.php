    ...
    if ($action == "insert") {
      ...
      if (mysqli_num_rows($checkEmail) > 0) {
        ...
        if ($data['password'] !== $data['re_password']) {
          ...
          $en_user = str_replace(".", "", $en_user);
          $to       = $data['email'];
          $subject  = "Account Verification - Portal Wisata Kafe";
                        ...
                        <tr>
                            ...
                            <table role='presentation' border='0' cellpadding='0' cellspacing='0' style='border-collapse: separate; mso-table-lspace: 0pt; mso-table-rspace: 0pt; min-width: 100%; width: 100%;' width='100%'>
                                ...
                                <td style='font-family: sans-serif; font-size: 14px; vertical-align: top;' valign='top'>
                                    ...
                                    <p style='font-family: sans-serif; font-size: 14px; font-weight: normal; margin: 0; margin-bottom: 15px;'>Terima kasih telah mendaftar di Portal Wisata Kafe.</p>
  ...
  function forgot_password($conn, $data, $action, $baseURL)
  {
    if ($action == "update") {
      ...
      if (mysqli_num_rows($checkEmail) === 0) {
        ...
        $en_user = str_replace(".", "", $en_user);
        $to       = $data['email'];
        $subject  = "Reset password - Portal Wisata Kafe";
        $message  = "<!doctype html>
        <html>
          <head>
              <meta name='viewport' content='width=device-width'>
...
if (isset($_SESSION["project_portal_wisata_kafe"]["users"])) {
    ...
    if ($action == "insert") {
      ...
      if (mysqli_num_rows($checkMenu) > 0) {
        ...
        if (!is_dir($pathFolder)) {
          ...
          fwrite($file, '<?php if (!isset($_SESSION["project_portal_wisata_kafe"]["users"])) {
  ...
  function sub_menu($conn, $data, $action, $baseURL)
  {
    ...
    if ($action == "insert") {
      ...
      if (mysqli_num_rows($checkSubMenu) > 0) {
        ...
        $_SESSION["project_portal_wisata_kafe"]["name_page"] = "' . $data['title'] . '";
        ...
        $_SESSION["project_portal_wisata_kafe"]["name_page"] = "Tambah ' . $data['title'] . '";
        require_once("../../templates/views_top.php"); ?>
        ...
        $_SESSION["project_portal_wisata_kafe"]["name_page"] = "Ubah ' . $data['title'] . '";
        require_once("../../templates/views_top.php"); ?>
        ...
        <div class="nxl-content" style="height: 100vh;">
          ...
          <div class="page-header">
            <div class="page-header-left d-flex align-items-center">
              ...
              <ul class="breadcrumb">
                <li class="breadcrumb-item">' . $data['title'] . '</li>
                <li class="breadcrumb-item"><?= $_SESSION["project_portal_wisata_kafe"]["name_page"].'.$petik.' '.$petik.'.$data_menu["menu"]  ?></li>
              </ul>
            </div>
          </div>
          <!-- [ page-header ] end -->
  ...
  function jam_operasional($conn, $data, $action)
  {
    if ($action == "insert") {
      $sql = "INSERT INTO jam_operasional (id_tempat,hari,jam_buka,jam_tutup) VALUES ('$data[id_tempat]','$data[hari]','$data[jam_buka]','$data[jam_tutup]')";
    }

    if ($action == "update") {
      $sql = "UPDATE jam_operasional SET hari = '$data[hari]', jam_buka='$data[jam_buka]', jam_tutup='$data[jam_tutup]' WHERE id_jam = '$data[id_jam]'";
    }

    if ($action == "delete") {
      $sql = "DELETE FROM jam_operasional WHERE id_jam = '$data[id_jam]'";
    }
